...
inline glm::mat4 Matrix_Rotate(float angle, glm::vec4 axis)
{
    ...
    return Matrix(
        ...
        vx*vx*(1-c)+c , vx*vy*(1-c)-(vz*s) , vx*vy*(1-c)+(vy*s) , 0.0f ,  // LINHA 1
...
inline glm::mat4 Matrix_Camera_View(glm::vec4 position_c, glm::vec4 view_vector, glm::vec4 up_vector)
{
    ...
    glm::vec4 u = crossproduct(up_vector, w)/norm(crossproduct(up_vector, w)) /* PREENCHA AQUI o cálculo do vetor u */;
...
// Realiza a multiplicação escalar entre um vetor v e um escalar e (v*e)
inline glm::vec4 scalarMultiplication(glm::vec4 v, double e){
    return glm::vec4(v.x*e, v.y*e, v.z*e, v.w);
}

// checa se dois dados vec4 possuem uma diferença menor do que 0.1 em todos os seus campos que não sejam o campo w
// usado para verificar se um objeto está perto o bastante de uma posição pré-definida sem causar comportamento inesperado por imprecisão de ponto flutuante
inline bool closeEnoughVectors(glm::vec4 v1, glm::vec4 v2){
    return (abs(v1.x - v2.x) < 0.1 && abs(v1.y - v2.y) < 0.1 && abs(v1.z - v2.z) < 0.1);
}
